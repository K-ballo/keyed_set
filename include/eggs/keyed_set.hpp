...
struct comparator : private Compare // EBO
{
    ...
    // Value vs Value
    constexpr bool operator()(Value const& a, Value const& b) const noexcept(
        noexcept(key_comp()(a.*Key, b.*Key))
    )
    {
    ...
    constexpr bool operator()(key_type const& k, Value const& b) const noexcept(
        noexcept(key_comp()(k, b.*Key))
    )
    {
    ...
    constexpr bool operator()(Value const& a, key_type const& k) const noexcept(
        noexcept(key_comp()(a.*Key, k))
    )
    {
    ...
    constexpr bool operator()(
        key_type const& a, key_type const& b
    ) const noexcept(noexcept(key_comp()(a, b)))
    {
        return key_comp()(a, b);
    }
    ...
    constexpr bool operator()(K const& k, Value const& b) const noexcept(
        noexcept(key_comp()(k, b.*Key))
    )
    {
        return key_comp()(k, b.*Key);
    ...
    template <typename K>
        requires requires { typename Compare::is_transparent; }
    constexpr bool operator()(Value const& a, K const& k) const noexcept(
        noexcept(key_comp()(a.*Key, k))
    )
    {
        return key_comp()(a.*Key, k);
...
template <
    ...
    auto Key,
    typename Compare =
        std::less<typename keyed_set_detail::member_key_type<Value, Key>::type>,
...
class keyed_set
{
  ...
  public:
    ...
    explicit keyed_set(
        key_compare const& c, allocator_type const& a = allocator_type{}
    )
    ...
    template <std::ranges::input_range R>
        requires std::
            convertible_to<std::ranges::range_reference_t<R>, value_type>
        keyed_set(
            std::from_range_t,
            R&& rg,
            key_compare const& c = key_compare{},
    ...
    void insert(std::initializer_list<value_type> il) { set_.insert(il); }
    ...
    node_type extract(const_iterator q) { return set_.extract(q); }
    ...
    iterator erase(const_iterator q) { return set_.erase(q); }
    ...
    void merge(keyed_set& other) { set_.merge(other.set_); }

    void merge(keyed_set&& other) { set_.merge(std::move(other.set_)); }

    // ── Observers ────────────────────────────────────────────────────────

    //! Returns the comparison object used to order keys.
    //! ([associative.reqmts.general]/p141).
    key_compare key_comp() const { return set_.key_comp().key_comp(); }

    //! Returns the projecting comparator that orders `value_type` objects.
    //! ([associative.reqmts.general]/p142).
    value_compare value_comp() const { return set_.key_comp(); }

    // ── Lookup ───────────────────────────────────────────────────────────

    const_iterator find(key_type const& k) const { return set_.find(k); }
    ...
    size_type count(key_type const& k) const { return set_.count(k); }
    ...
    bool contains(key_type const& k) const { return set_.contains(k); }
    ...
    void swap(keyed_set& other) noexcept { set_.swap(other.set_); }
    ...
    friend void swap(keyed_set& a, keyed_set& b) noexcept { a.swap(b); }
