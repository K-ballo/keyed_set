...
TEST_CASE("equal_range(key_type const&) — absent key between elements, empty range",
          "[keyed_set.lookup]")
{
    M m{{1, "A"}, {3, "C"}};
...
TEST_CASE("equal_range(key_type const&) — empty container returns [end, end)",
          "[keyed_set.lookup]")
{
    M m;
...
TEST_CASE("equal_range(key_type const&) — key below all elements returns [begin, begin)",
          "[keyed_set.lookup]")
{
    M m{{5, "E"}, {10, "J"}, {15, "O"}};
...
TEST_CASE("equal_range(key_type const&) — key above all elements returns [end, end)",
          "[keyed_set.lookup]")
{
    M m{{5, "E"}, {10, "J"}, {15, "O"}};
...
TEST_CASE("equal_range — consistent with lower_bound and upper_bound",
          "[keyed_set.lookup]")
{
